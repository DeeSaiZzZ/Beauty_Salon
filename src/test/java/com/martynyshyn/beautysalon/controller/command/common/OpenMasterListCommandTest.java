...
class OpenMasterListCommandTest {

    @InjectMocks
    OpenMasterListCommand command;
    ...
    OpenMasterListCommandTest() throws SQLException {
    }

    @Test
    void openMasterListTest() {
        try (MockedStatic<ConnectionPool> cp = Mockito.mockStatic(ConnectionPool.class)) {
            ...
            String query = "SELECT id, email, firstName, lastName, rate, speciality_id FROM employee";
            ...
            try (MockedStatic<QueryCreator> qc = Mockito.mockStatic(QueryCreator.class)) {
                ...
                assertEquals(Url.MASTER_LIST_PAGE, result);
            }
        }
    }
}
